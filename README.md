...
- Supports `cscope`, `gtags-cscope` or `auto`.
  - `auto` option choose between `cscope` and `gtags-cscope` based on DB type. If no DB is found, `cscope` is used.
...
```lua
{
  ...
  cscope = {
    ...
    -- cscope executable
    exec = "cscope", -- "cscope", "gtags-cscope" or "auto"
